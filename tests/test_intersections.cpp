...
TEST(intersections, valid_types) {
    const Shape line_1(Line{});
    const Shape line_2(Line{});
    const Shape circle_1(Circle({0, 0}, 1));
    const Shape circle_2(Circle({0, 0}, 1));

    EXPECT_NO_THROW(std::visit(IntersectionVisitor{}, line_1, line_2));
    EXPECT_NO_THROW(std::visit(IntersectionVisitor{}, line_1, circle_1));
    EXPECT_NO_THROW(std::visit(IntersectionVisitor{}, circle_1, line_2));
    EXPECT_NO_THROW(std::visit(IntersectionVisitor{}, circle_1, circle_2));
}
...
TEST(intersections, unexpected_type) {
    ...
    EXPECT_ANY_THROW(std::visit(IntersectionVisitor{}, unexpected_shape_1, unexpected_shape_2));
    EXPECT_ANY_THROW(std::visit(IntersectionVisitor{}, valid_type, unexpected_shape_2));
    EXPECT_ANY_THROW(std::visit(IntersectionVisitor{}, unexpected_shape_1, valid_type));
}
